...
* Classifiers: Six classifiers are evaluated in order to identify the most accurate one. 
...
We additionally perform a s summary of these subsets in order to check if there are missing values.
...
test_index <- createDataPartition(seeds_x$Perimeter, times = 1, p = 0.2, list = FALSE) # Create the partition
...
```{r algorithms, echo = TRUE, warning = FALSE}
...
# We create a dataframe containing the results of each algorithm. It is necessary to convert every prediction from factor to numeric.
...
results <- bind_rows(results, data_frame(Algorithm = "Ensemble", Accuracy = ensemple_accuracy )) # Save results
...
We therefore conclude that the highest accuracy was 0.8649559 using the *Regularized movie and user effect model*.
